...
class LoggerService:
    ...
    def log(self, message):
        logger.info(
            f"{self.get_date()} LOG [{self.service_name}] {self.message_parser(message)}"
        )

    def error(self, message):
        logger.error(
            f"{self.get_date()} ERR [{self.service_name}] {self.message_parser(message)}"
        )

    def debug(self, message):
        logger.debug(
            f"{self.get_date()} DEBUG [{self.service_name}] {self.message_parser(message)}"
        )
